...
export async function migrate69Wilayas() {
  console.log('[Migration] Ensuring all 69 Algerian Wilayas are configured in DeliverySetting...');
  
  let deliverySetting = await DeliverySetting.findOne();
  if (!deliverySetting) {
    console.log('[Migration] No DeliverySetting found. Creating fresh 69-wilaya dataset...');
  ...
  const mergedRates = ALGERIA_WILAYAS.map(canonical => {
    if (existingMap.has(canonical.code)) {
      const existing = existingMap.get(canonical.code);
      // Preserve existing store owner fees while updating canonical names
      return {
  ...
  deliverySetting.wilayaRates = mergedRates;
  await deliverySetting.save();

  console.log(`[Migration] Success! Total Wilayas in DB: ${deliverySetting.wilayaRates.length} (New wilayas added: ${addedCount}).`);
  return deliverySetting;
}

// Standalone execution support
if (process.argv[1]?.endsWith('migrate69Wilayas.js')) {
  (async () => {
    try {
